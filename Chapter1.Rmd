...
Second thing to know is detrending. Detrending is necessary to remove variability resulted from "trend" over time (oftentimes using diff()). Think of this as standardized for analysis. Sometimes the data needs log transformation before detrending (more on this in future chapter)
...
```{r echo=TRUE}
WN_example <- arima.sim(model = list(order = c(0,0,0)), n = 50)
ts.plot(WN_example)
```

For now, just know that the c(0,0,0) specifies the WN model. 
...
Again, just know that 0,1,0 specifies RW. 
